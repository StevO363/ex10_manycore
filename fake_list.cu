  ...
  for (int dude = blockIdx.x * blockDim.x + threadIdx.x; dude < population_size;
    ...
    if (cuda_fakenews_believe_strength[dude] > 0.5) {
      for (int contact = 0; contact < contacts_today; ++contact) {
        double r =
            cuda_rand_values_threads[tid_global * max_rand_val_per_thread];
        if (r < transmission_probability_today) {
          r = cuda_rand_values_threads[tid_global * max_rand_val_per_thread +
                                       contact + 1];
...
void run_simulation(const SimInput_t *input, SimOutput_t *output) {
  ...
  int max_rand_val_per_thread =
      (input->population_size + num_blocks * num_threads_per_block - 1) /
      (num_blocks * num_threads_per_block) * 2 * max_contacts_per_day;
  ...
  double *tmp_rand_num_array =
      new double[max_rand_val_per_thread * num_blocks * num_threads_per_block];
  ...
  double *cuda_rand_values_threads;
  cudaMalloc(&cuda_rand_values_threads, sizeof(double) * num_blocks *
  ...
  for (int day = 0; day < 365; ++day) {
    ...
    generate_random_seq(tmp_rand_num_array, num_blocks * num_threads_per_block *
                                                max_rand_val_per_thread);
    cudaMemcpy(cuda_rand_values_threads, tmp_rand_num_array,
               sizeof(double) * num_blocks * num_threads_per_block *
                   max_rand_val_per_thread,
               cudaMemcpyHostToDevice);

    // use cuda kernel for computation of populatioin loop
    cuda_pass_on_fakenews<<<num_blocks, num_threads_per_block>>>(
        input->population_size, cuda_rand_values_threads,
        cuda_fakenews_believe_strength, recovery_rate_today,
        transmission_probability_today, contacts_today,
        max_rand_val_per_thread);
  }
